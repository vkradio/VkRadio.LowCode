...
{
    public abstract class DBSchemaMetaModelJson
    {
        protected MetaModel.MetaModel _metaModel;
        protected ArtefactGeneratorSqlJson _artefactGeneratorSql;
        protected SchemaDeploymentScriptJson _schemaDeploymentScript;
        protected Dictionary<Guid, TableAndSourceCorrespondenceJson> _tableAndSourceCorrespondence = new Dictionary<Guid,TableAndSourceCorrespondenceJson>();
        protected bool _supportsForeignKeyConstraints;

        protected abstract TableJson CreateTable(DOTDefinition in_dotDefinition);
        ...
        protected abstract ValueFieldJson CreateTableFieldValue(TableAndDOTCorrespondenceJson in_correspondense, PropertyDefinition in_propertyDefinition);
        protected abstract ForeignKeyFieldJson CreateForeignKeyField(TableAndDOTCorrespondenceJson in_correspondense, PropertyDefinition in_propertyDefinition);
        protected abstract PredefinedInsertJson CreatePredefinedInsert();
        protected abstract FieldValueKeyJson CreateFieldValueKey(PredefinedInsertJson in_predefinedInsert, ITableFieldJson in_field, Guid in_value);
        ...
        protected abstract SchemaDeploymentScriptJson CreateSchemaDeploymentScript();

        public MetaModel.MetaModel MetaModel { get { return _metaModel; } }
        public ArtefactGeneratorSqlJson ArtefactGeneratorSql { get { return _artefactGeneratorSql; } }
        public SchemaDeploymentScriptJson SchemaDeploymentScript { get { return _schemaDeploymentScript; } }
        public IDictionary<Guid, TableAndSourceCorrespondenceJson> TableAndSourceCorrespondence { get { return _tableAndSourceCorrespondence; } }
        /// <summary>
        ...
        public static string GetReferencedTableName(PropertyCorrespondenceJson in_propertyCorrespondence, IDictionary<Guid, TableAndSourceCorrespondenceJson> in_tableAndSourceCorrespondence)
        {
            string refTableName = null;
            PFTReferenceValue pftRefVal = in_propertyCorrespondence.PropertyDefinition.FunctionalType as PFTReferenceValue;
            if (pftRefVal != null)
            {
                refTableName = ((TableAndDOTCorrespondenceJson)in_tableAndSourceCorrespondence[pftRefVal.RelationshipReference.ReferenceDefinition.Id]).Table.Name;
            }
            else
            {
                PFTTableOwner pftTableOwner = in_propertyCorrespondence.PropertyDefinition.FunctionalType as PFTTableOwner;
                if (pftTableOwner != null)
                {
                    refTableName = ((TableAndDOTCorrespondenceJson)in_tableAndSourceCorrespondence[pftTableOwner.RelationshipTable.PropertyDefinitionInOwner.OwnerDefinition.Id]).Table.Name;
                }
                else
                {
                    PFTConnector pftConnector = in_propertyCorrespondence.PropertyDefinition.FunctionalType as PFTConnector;
                    if (pftConnector != null)
                    {
                        PropertyDefinition otherEnd = pftConnector.RelationshipConnector.End1.PropertyDefinition.Id == in_propertyCorrespondence.PropertyDefinition.Id ?
                            pftConnector.RelationshipConnector.End2.PropertyDefinition :
                            pftConnector.RelationshipConnector.End1.PropertyDefinition;
                        refTableName = ((TableAndDOTCorrespondenceJson)in_tableAndSourceCorrespondence[otherEnd.OwnerDefinition.Id]).Table.Name;
                    }
                    else
                    {
        ...
        public void Init()
        {
            ...
            {
                TableJson tableDef = CreateTable(dotDef);
                
                TableAndDOTCorrespondenceJson correspondence = new TableAndDOTCorrespondenceJson() { DBSchemaMetaModel = this, DOTDefinition = dotDef, Table = tableDef };
                ...
                {
                    ITableFieldJson field = null;

                    // Свойство хранит непосредственное значение?
                    if (!(propDef.FunctionalType is PFTLink))
                    {
                        ValueFieldJson vf = CreateTableFieldValue(correspondence, propDef);
                    ...
                    else
                    {
                        ...
                        if (!(propDef.FunctionalType is PFTTablePart || propDef.FunctionalType is PFTBackReferencedTable))
                        {
                            ...
                            tableDef.ForeignKeyFields.Add((ForeignKeyFieldJson)field);
                        }
                    }
            ...
            foreach (DOTDefinition dotDef in _metaModel.AllDOTDefinitions.Values)
            {
                ...
                {
                    TableAndDOTCorrespondenceJson corr = (TableAndDOTCorrespondenceJson)_tableAndSourceCorrespondence[dotDef.Id];
                    
                    PredefinedInsertJson pi = CreatePredefinedInsert();
                    pi.SchemaDeploymentScript = _schemaDeploymentScript;
                    pi.Table = corr.Table;
                    _schemaDeploymentScript.PredefinedInserts.Add(pi);

                    pi.FieldValues.Add(CreateFieldValueKey(pi, (PKSingleJson)corr.Table.PrimaryKey, pdo.Id));

                    foreach (PropertyCorrespondenceJson pCorr in corr.PropertyCorrespondences)
                    {
                        object valueObject = pdo.PropertyValues[pCorr.PropertyDefinition.Id].ValueObject;
                        ...
                        FieldValueJson fv = new FieldValueJson()
                        {
                            PredefinedInsert = pi,
            ...
            _schemaDeploymentScript.PredefinedInserts.Sort(PredefinedInsertJson.PredefinedInsertComparer);
            #endregion

            #region 4. Генерирование связок по внешним ключам.
            if (_supportsForeignKeyConstraints)
            {
                foreach (TableAndSourceCorrespondenceJson srcCorr in _tableAndSourceCorrespondence.Values)
                {
                    TableAndDOTCorrespondenceJson corr = srcCorr as TableAndDOTCorrespondenceJson;
                    if (corr == null)
                        continue;
                    foreach (PropertyCorrespondenceJson propCorr in corr.PropertyCorrespondences)
                    {
                        if (propCorr.PropertyDefinition == null)
                            continue;
                        IPFTDependentLink dependentLink = propCorr.PropertyDefinition.FunctionalType as IPFTDependentLink;
        ...
        public DBSchemaMetaModelJson(MetaModel.MetaModel in_metaModel, ArtefactGeneratorSqlJson in_artefactGeneratorSql)
        {
