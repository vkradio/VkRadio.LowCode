...
namespace ArtefactGenerationProject.ArtefactGenerator.Ool.CSharp.Classic.Package.Model
{
    public class DOTPackage: PackNS.Package
    {
        ...
        {
            in_method.BodyStrings.Add(string.Format("{0} clone = new {1}();", in_method.Class.Name, in_method.Class.Name));
            in_method.BodyStrings.Add("CloneBase(this, clone);");

            foreach (PropertyDefinition propDef in in_dotDef.PropertyDefinitions.Values)
            {
                string cloneVarString = GenerateCloneVariableForProp(propDef);
        ...
        {
            in_method.BodyStrings.Add("string baseResult = base.Validate();");
        ...
        static string[] GenerateValidatesStringsForPropDef(PropertyDefinition in_propDef)
        {
            ...
            string varName = "_" + NameHelper.NamesToCamelCase(in_propDef.Names);
            string propLocalName = NameHelper.GetLocalNameUpperCase(in_propDef.Names);
        ...
        static string[] GenerateValidateVarValueForString(PropertyDefinition in_propDef, PFTString in_pftString, string in_varName, string in_propLocalName)
        {
            ...
            if (!in_pftString.Nullable)
            {
                text.Add(string.Format("if (string.IsNullOrWhiteSpace({0}))", in_varName));
                text.Add(string.Format("    return string.Format(c_propertyValueNotSet, \"{0}\");", in_propLocalName));
            }

            int minLength = in_pftString.MinLength;
            if (minLength == 0 && !in_pftString.Nullable)
                minLength = 1;

            string nullableEquation = string.Empty;
            if (in_pftString.Nullable)
                nullableEquation = string.Format("{0} != null && ", in_varName);

            string minEquation = string.Empty;
            if (minLength > 0)
                minEquation = string.Format("{0}{1}.Length < {2} || ", (in_pftString.Nullable && minLength > 0 ? "(" : string.Empty), in_varName, minLength);

            string equation = string.Format("if ({0}{1}{2}.Length > {3})", nullableEquation, minEquation, in_varName, in_pftString.MaxLength);
            if (in_pftString.Nullable && minLength > 0)
                equation += ")";
            text.Add(equation);
            text.Add(string.Format("    return string.Format(c_invalidPropertyLength, \"{0}\", {1}, {2});", in_propLocalName, minLength, in_pftString.MaxLength));
        ...
        static string[] GenerateValidateVarValueForDateTime(PropertyDefinition in_propDef, PFTDateTime in_pftDateTime, string in_varName, string in_propLocalName)
        {
            ...
            if (in_pftDateTime.Nullable)
                text.Add(string.Format("if ({0}.HasValue && ({0}.Value < C_MIN_SQL_DATE_TIME || {0}.Value > C_MAX_SQL_DATE_TIME))", in_varName));
            else
                text.Add(string.Format("if ({0} < C_MIN_SQL_DATE_TIME || {0} > C_MAX_SQL_DATE_TIME)", in_varName));
            text.Add(string.Format("    return string.Format(c_invalidPropertyDateTime, \"{0}\");", in_propLocalName));
        ...
        static string[] GenerateValidateVarValueForRefValue(PropertyDefinition in_propDef, PFTLink in_pftRef, string in_varName, string in_varNameWOId, string in_propLocalName)
        {
            ...
            if (!in_pftRef.Nullable)
            {
                text.Add(string.Format("if (!{0}Id.HasValue && {0} == null)", in_varNameWOId));
                text.Add(string.Format("    return string.Format(c_propertyValueNotSet, \"{0}\");", in_propLocalName));
            }
        ...
        {
            List<PropertyDefinition> refProps = new List<PropertyDefinition>();
            ...
            if (refProps.Count > 0)
            {
                ...
                foreach (PropertyDefinition propDef in refProps)
        ...
        static void CreateMethodToString(CSClass in_class, DOTDefinition in_dotDef)
        {
            ...
            string gettingWOverrider = $"_overrider != null && _overrider.OverrideToString ? (_overrider.ToStringOverride()) : ({getting})";
        ...
        public static CSClass CreateDOTClass(Component.CSComponent component, DOTDefinition dotDef)
        {
            ...
            foreach (var propDef in dotDef.PropertyDefinitions.Values)
            {
                ...
                FieldPropertyHelper.GenerateFieldPropertyAndGetter(propDef, result, out field, out fieldId, out prop, out propId, out getter, out counter);
                if (field != null)
