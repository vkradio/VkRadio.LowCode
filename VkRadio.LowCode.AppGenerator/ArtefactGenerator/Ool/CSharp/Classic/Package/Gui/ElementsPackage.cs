...
{
    Dictionary<string, CSComponentWMainClass> _mainComponents = [];
    Dictionary<string, ComponentWPredefinedCode> _designerComponents = [];
    Dictionary<string, ComponentWPredefinedCode> _resxComponents = [];
    ...
    #region Generate a main component of an element card
    /// <summary>
    /// Create a string of a method call for calculating of table objects amount
    /// </summary>
    /// <param name="propertyName">Document object type class property</param>
    /// <returns></returns>
    ...
    )
    {
        ...
        foreach (var propDef in dotDef.PropertyDefinitions.Values)
        {
            ...
            {
                var propClassName = NameHelper.NamesToPascalCase(propDef.Names);
    ...
    private static void GenerateMethodSyncToDOT(CSClass @class, DOTDefinition dotDef, List<CSharpHelper.PropertyWidgetDescriptor> widgetDescs)
    {
        ...
        foreach (var wDesc in widgetDescs)
        {
            if (wDesc.WidgetName.Substring(0, 3) == "SF_")
            {
                ...
                var typeNameForMethod = string.Empty;
    ...
    private static void GenerateMethodsForClearProps(CSClass @class, List<CSharpHelper.PropertyWidgetDescriptor> clearProps)
    {
        ...
        foreach (var wDesc in clearProps)
        {
            var method = new CSMethod
            {
                Class = @class,
    ...
    private static void GenerateMethodsForSelectProps(CSClass @class, List<CSharpHelper.PropertyWidgetDescriptor> selectProps)
    {
        ...
        foreach (var wDesc in selectProps)
        {
            var method = new CSMethod
            {
                Class = @class,
    ...
    private static void GenerateMethodsForListProps(CSClass @class, List<CSharpHelper.PropertyWidgetDescriptor> listProps)
    {
        ...
        foreach (var wDesc in listProps)
        {
            var method = new CSMethod
            {
                Class = @class,
            ...
            if (backRefTable is not null)
            {
                fieldName = NameHelper.NameToUnderscoreSeparatedName(backRefTable.RelationshipReference.OwnerPropertyDefinition.Names);
            }
            else
            {
                var tablePart = wDesc.PropertyDefinition.FunctionalType as PFTTablePart;

                if (tablePart is not null)
                {
    ...
    private static void GenerateMethodsForCardProps(CSClass @class, List<CSharpHelper.PropertyWidgetDescriptor> cardProps)
    {
        ...
        foreach (var wDesc in cardProps)
        {
            var method = new CSMethod
            {
                Class = @class,
    ...
    #region Generating a *.Designer component of an element card
    private static void GenerateWidgetConstructions(IList<string> predefinedCode, List<CSharpHelper.PropertyWidgetDescriptor> widgetDescs)
    {
        foreach (var wDesc in widgetDescs)
        {
            predefinedCode.Add(string.Format("            this.{0} = new {1}();", wDesc.WidgetName, wDesc.WidgetType));
        }
    }

    private static int SetupWidgetParameters(IList<CSharpHelper.PropertyWidgetDescriptor> widgetDescs, IList<string> predefinedCode)
    {
        ...
        foreach (var wDesc in widgetDescs)
        {
            var isBoolProp = wDesc.WidgetName.IndexOf("CHK_") == 0;
            var canSelectString = wDesc.CanSelect ? "true" : "false";
            var captionPropName = isBoolProp ? "Text" : "Caption";
            var isMultiline = wDesc.PropertyDefinition.FunctionalType is PFTText;

            var widgetHeight = isMultiline
            ...
            {
                predefinedCode.Add(string.Format("            this.{0}.EnabledCard = {1};", wDesc.WidgetName, canSelectString));
                predefinedCode.Add(string.Format("            this.{0}.EnabledClear = {1};", wDesc.WidgetName, canSelectString));
                predefinedCode.Add(string.Format("            this.{0}.EnabledSelect = true;", wDesc.WidgetName));
                predefinedCode.Add(string.Format("            this.{0}.EnabledQuickSelect = {1};", wDesc.WidgetName, canSelectString));
            }

            predefinedCode.Add(string.Format("            this.{0}.Location = new System.Drawing.Point({1}, {2});", wDesc.WidgetName, isBoolProp ? c_checkBoxOffset : 0, widgetTop));
            predefinedCode.Add(string.Format("            this.{0}.Name = \"{0}\";", wDesc.WidgetName));

            if (isMultiline)
            {
                predefinedCode.Add(string.Format("            this.{0}.Multiline = true;", wDesc.WidgetName));
                predefinedCode.Add(string.Format("            this.{0}.Anchor = System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Right;", wDesc.WidgetName));
            }

            if (wDesc.PropertyDefinition.FunctionalType is PFTMoney)
            {
                var pftMoney = (PFTMoney)wDesc.PropertyDefinition.FunctionalType;
                predefinedCode.Add(string.Format("            this.{0}.TreatAsDecimal = true;", wDesc.WidgetName));
            ...
            if (wDesc.WidgetName.IndexOf("SEL_") == 0)
            {
                predefinedCode.Add(string.Format("            this.{0}.UseCard = {1};", wDesc.WidgetName, canSelectString));
                predefinedCode.Add(string.Format("            this.{0}.UseClear = {1};", wDesc.WidgetName, canSelectString));
                predefinedCode.Add(string.Format("            this.{0}.UseSelect = true;", wDesc.WidgetName));
                predefinedCode.Add(string.Format("            this.{0}.UseQuickSelect = {1};", wDesc.WidgetName, canSelectString));
            }

            if (!isBoolProp)
            {
                predefinedCode.Add(string.Format("            this.{0}.Value = \"\";", wDesc.WidgetName));
            }
        ...
        : base(parentPackage, "Elements")
    {
        ...
        foreach (var dotDef in dotDefs)
        {
            ...
            var panComponent = new CSComponentWMainClass
            {
                Package = this,
                Name = formName + ".cs",
                DOTDefinition = dotDef,
            ...
            var designerComponent = new ComponentWPredefinedCode
            {
                Package = this,
                EmitUtf8Bom = true,
            ...
            // 1. Create widgets for properties
            ...
            // 2. Setting up parameters of properties
            ...
            // 3. Add widgets to a form
            foreach (var wDesc in wDescs)
            {
            ...
            // 4. Declare class fields for widgets
            foreach (var wDesc in wDescs)
            {
            ...
            #region Card resource component
            var resxComponent = new CardResourceComponent(this, formName + ".resx");
