...
namespace ArtefactGenerationProject.ArtefactGenerator.Ool.CSharp.Modular.Component
{
    ...
    {
        string[] GetConfigBatch(Guid projectId) => GetConfigBatch(projectId.ToString().ToUpper());

        string[] GetConfigBatch(string projectId)
        {
        ...
        public Solution(CSharpSolution in_rootPackage)
        {
            ...
            _predefinedCode.Add($"Project(\"{{9A19103F-16F7-4668-BE54-9A1E7A4F7556}}\") = \"{name}_base\", \"base\\{name}_base.csproj\", \"{{{in_rootPackage.BaseProject.ProjectGuid.ToString().ToUpper()}}}\"");
            ...
            //_predefinedCode.Add($"EndProject");
            _predefinedCode.Add($"Project(\"{{9A19103F-16F7-4668-BE54-9A1E7A4F7556}}\") = \"{cSharpAppTarget.OrmLibProjectNameDb}\", \"{FileHelper.GetRelativePath(in_rootPackage.FullPath + "\\", cSharpAppTarget.GetOrmLibProjectFilePathDb())}\", \"{{{TargetCSharpApp.C_ORMLIB_PROJECT_DB_GUID_STRING}}}\"");
            _predefinedCode.Add($"EndProject");
            _predefinedCode.Add($"Project(\"{{9A19103F-16F7-4668-BE54-9A1E7A4F7556}}\") = \"{cSharpAppTarget.OrmLibProjectNameDbConcrete}\", \"{FileHelper.GetRelativePath(in_rootPackage.FullPath + "\\", cSharpAppTarget.GetOrmLibProjectFilePathDbConcrete())}\", \"{{{TargetCSharpApp.C_ORMLIB_PROJECT_DB_CONCRETE_GUID_STRING}}}\"");
            _predefinedCode.Add($"EndProject");
            _predefinedCode.Add($"Project(\"{{9A19103F-16F7-4668-BE54-9A1E7A4F7556}}\") = \"{cSharpAppTarget.OrmLibProjectNameUtil}\", \"{FileHelper.GetRelativePath(in_rootPackage.FullPath + "\\", cSharpAppTarget.GetOrmLibProjectFilePathUtil())}\", \"{{{TargetCSharpApp.C_ORMLIB_PROJECT_UTIL_GUID_STRING}}}\"");
            _predefinedCode.Add($"EndProject");
