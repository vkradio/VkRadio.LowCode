    ...
    public class Storage: CSComponentWMainClass
    {
        static List<CSClassConstant> GenerateFieldConstants(TableAndDOTCorrespondenceJson tableAndDotCorrespondence, CSClass storageClass, List<int> decimalConstants)
        {
            ...
            foreach (var field in tableAndDotCorrespondence.Table.AllFields)
            {
                ...
                {
                    Class = storageClass,
                    DocComment = new XmlComment($"Табличное поле {NameHelper.GetLocalNameUpperCase(field.DOTPropertyCorrespondence.PropertyDefinition.Names)} (вариант без кавычек)"),
                ...
                {
                    Class = storageClass,
                    DocComment = new XmlComment($"Табличное поле {NameHelper.GetLocalNameUpperCase(field.DOTPropertyCorrespondence.PropertyDefinition.Names)} (вариант с кавычками)"),
        ...
        static void GenerateConstructor(List<CSClassConstant> fieldConstants, CSClass storageClass, string dotClassName, string storageClassName, DOTDefinition dotDef, TableAndDOTCorrespondenceJson tableAndDotCorrespondence, List<int> decimalConstants)
        {
        ...
        static void GenerateFillDOFromReader(CSClass storageClass, string dotClassName, TableAndDOTCorrespondenceJson tableAndDotCorrespondence)
        {
            #region Шапка метода.
            ...
            for (var i = 1; i < tableAndDotCorrespondence.Table.AllFields.Count; i++)
            {
                ...
                PropertyCorrespondenceJson corr = tableAndDotCorrespondence.PropertyCorrespondences.SingleOrDefault(c => c.TableField.Name == field.Name);
        ...
        static void GenerateFillParameters(CSClass storageClass, string dotClassName, TableAndDOTCorrespondenceJson tableAndDotCorrespondence)
        {
            #region Шапка метода.
            var method = new CSMethod
            {
        ...
        static void GenerateRestoreByNameMethod(CSClass storageClass, string dotClassName, TableAndDOTCorrespondenceJson tableAndDotCorrespondence)
        {
