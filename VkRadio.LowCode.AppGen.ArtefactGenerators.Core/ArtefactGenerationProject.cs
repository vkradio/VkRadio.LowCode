...
public class ArtefactGenerationProject : IUnique
{
    ...
    public static ArtefactGenerationProject Load(string filePath, Func<ArtefactTypeEnum, DomainModel, Target, ArtefactGenerator> artefactGeneratorConstructor)
    {
        ...
        {
            project.Targets.Add(Target.LoadFromXElement(project, xel, artefactGeneratorConstructor));
        }
    ...
    public string? GenerateArtefacts()
    {
        ...
        foreach (var target in Targets)
        {
            ...
            foreach (var depTarget in target.DependsOn)
            {
                if (!depTarget.GenerationSucceeded)
                {
